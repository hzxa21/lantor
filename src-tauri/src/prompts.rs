...
Use task_create only for durable globally tracked work. Use task_handoff when you are the current task assignee and need to transfer that task to another agent with a reason; omit task_number only when the current turn is tied to the task. Use handoff_create only to transfer a concrete existing thread to another agent after clear user authorization; it is not a general cross-thread messaging API. Use channel_message_create only after the user explicitly asks you to post a message in a specific channel/thread; it posts as your agent identity, requires channel membership, and normal @mentions may dispatch work. Use channel_create for durable topic workspaces, multi-agent collaboration, recurring follow-up, or explicit user requests to open a new channel; include a clear description and invite relevant agents. Use artifact_create only for long markdown reports that should render in the thread; keep the visible chat summary short. Use attachment_create for generated images or local files that should appear as message attachments; pass absolute file paths, not base64. Do not use artifact_create for HTML, SVG, Mermaid, flowchart DSL, charts, or interactive previews."#
}

fn streaming_reply_contract_prompt(runtime_name: &str) -> String {
    format!(
        "Reply normally only when a visible response is useful. Lantor will stream your {runtime_name} assistant text into the correct channel/thread automatically. Reply briefly to direct greetings, low-intent testing messages, or \"are you there?\" checks so the user can tell you are alive. If the latest user message is only a pure acknowledgement, thanks, emoji-only message, or non-actionable chatter that does not need a response, output exactly `LANTOR_SILENT_REPLY: <short reason>` and nothing else. Keep visible thread messages high-density: final results, decisions, blockers, user questions, and handoffs only. Do not narrate every intermediate step in chat. In warm streaming mode you may emit standalone LANTOR_EVENT control lines for activity, reminders, memory, profile, channel, artifact_create, attachment_create, channel_message_create, handoff_create, task_handoff, usage, durable task_create, or task_status; Lantor consumes those control lines and hides them from chat. Treat channel_message_create as a user-authorized way to post a normal agent message into a specific channel/thread, not as a background notification API. Treat task_handoff as the controlled way for the current assignee to transfer an active task to another agent with a reason. Treat handoff_create as a constrained transfer of one existing thread to another agent after clear user authorization, not a general message API. Treat channel_create as a normal tool for durable topics, multi-agent collaboration, recurring follow-up, or explicit user requests to open a new channel. Do not emit LANTOR_EVENT message/task_claim lines in this streaming mode unless explicitly asked to debug the stdout command path."
    )
}

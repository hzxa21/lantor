...
- Agent runtime runs with start/stop controls, process status, and persisted run logs in Postgres.
...
Current runtime boundary: each agent profile can store a shell `launch_command` and optional `working_directory`. If the command is empty, LocalSlock starts a harmless placeholder process so the start/stop/log loop can be tested before wiring a real agent CLI. A desktop app restart marks previously running runs as `unknown`, because the MVP does not yet include a separate always-on supervisor process.
